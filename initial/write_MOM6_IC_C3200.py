...
def write_initial(config):
   ...
   # Define the longitude and latitude range
    ...
    print(f"  min lat: {lat_min}")
    print(f"  max lon: {lon_max}")
    print(f"  min lon: {lon_min}")
...
#    input("Press Enter to continue...")  # Pauses execution
    ...
    xh_1d = target_grid['x'].isel(nxp=slice(1, None, 2), nyp=0).values
    yh_1d = target_grid['y'].isel(nxp=0, nyp=slice(1, None, 2)).values
    xq_1d = target_grid['x'].isel(nxp=slice(0, None, 2), nyp=0).values
    yq_1d = target_grid['y'].isel(nxp=0, nyp=slice(0, None, 2)).values
...
# === Pause ===
    ...
    if 'temp' in encodings:
        encodings['temp']['_FillValue'] = fill
    if 'salt' in encodings:
        encodings['salt']['_FillValue'] = fill
