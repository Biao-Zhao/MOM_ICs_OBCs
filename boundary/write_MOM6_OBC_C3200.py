...
1. Process single-day output:
   ./write_glorys_boundary_day.py --config config.yaml --year <YEAR> --month <MONTH> --day <DAY> --hour <HOUR>
...
# author: 'Jing Chen'
# modified by 'Biao Zhao'
# description: 'Boundary conditions for MOM6, generated from GLORYS PHY fields'
# created: '2025-08-05'
...
def write_day(date, glorys_dir, segments, variables, output_prefix):
    ...
    filename = f"{output_prefix}_{date:%Y%m%d}-{date:%H}.nc"
    ...
    for segment in segments:
        for variable in variables:
            if variable == 'uv':
                ...
                segment.regrid_velocity(glorys['uo'], glorys['vo'], suffix=f"{date:%Y%m%d%H}", flood=False,
                                        time_attrs=time_attrs, time_encoding=time_encoding )
            elif variable in ['thetao', 'so', 'zos']:
                print(f"Processing {segment.border} {variable}")
                segment.regrid_tracer(glorys[variable], suffix=f"{date:%Y%m%d%H}", flood=False,
                                      time_attrs=time_attrs, time_encoding=time_encoding)
...
def process_single_day(config, year, month, day, hour):
    """Process data for a single day."""
    specific_date = datetime(year, month, day, hour)
    print(f"Processing data for {specific_date}...")

    glorys_dir = config['glorys_dir']
    output_prefix = config.get('_OUTPUT_PREFIX', 'merged')
...
def main():
    ...
    parser.add_argument('--hour', type=int, required=True, help="Hour (00/06/12/18) for single-day processing")
    ...
    elif args.year and args.month and args.day and (args.hour is not None):
        process_single_day(config, args.year, args.month, args.day, args.hour)
    else:
        print("Error: Specify either --ncrcat_years or a specific date (--year, --month, --day, --hour).")
