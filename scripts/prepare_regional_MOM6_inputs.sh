...
# Crop a domain for dowloading Re/Analysis data. It should be larger than the model domain.
MIN_LON=-105
MAX_LON=-35
MIN_LAT=10
MAX_LAT=55
...
while [[ "$CURRENT_DATE" != "$END_NEXT" ]]; do
  for HOUR in "${TIME_SLOTS[@]}"; do
    ...
    YAML_MERGE="${WORK_DIR}/scripts/boundary/config_merge_glorys.yaml"
    mkdir -p ${WORK_DIR}/scripts/boundary  
    mkdir -p ${WORK_DIR}/scripts/temp/${DATE_COMPACT}/
...
uovo_fn: ${UOVO_PATH}
ssh_fn: ${SSH_PATH}
merged_fn: ${WORK_DIR}/scripts/temp/${DATE_COMPACT}/merged_${DATE_COMPACT}-${HOUR}.nc
ssh_time: ${HOUR}
min_lon: ${MIN_LON}
max_lon: ${MAX_LON}
min_lat: ${MIN_LAT}
...
EOF
    ...
    YAML_REGRID="${WORK_DIR}/scripts/boundary/config_regrid_glorys.yaml"

    echo "[INFO] Writing ${YAML_REGRID}"
{
cat <<EOF
glorys_dir: ${WORK_DIR}/scripts/temp/${DATE_COMPACT}
output_dir: ${WORK_DIR}/scripts/temp/
hgrid: ${HGRID_FILE}
resolution: ${res}
...
EOF
  ...
  for var in "${VARS[@]}"; do
    for seg in "${SEGS[@]}"; do
      ...
      mapfile -t files < <(find "${WORK_DIR}/scripts/temp/" -maxdepth 1 -type f -name "${var}_${segnum}_*.nc" | sort -V)
      if (( ${#files[@]} == 0 )); then
        echo "[ERROR] No input files for ${var}_${segnum} in ${WORK_DIR}/scripts/temp/."
